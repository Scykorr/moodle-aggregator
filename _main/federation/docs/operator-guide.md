...
|---|---|---|---|
| Агрегатор | `compose.aggregator.yml` | `moodle-aggregator-aggregator-1` | Публичный каталог (факультеты/кафедры), админка, проверка HTTP(S) |
...
**Keycloak не проверяет доступность Moodle и не редактирует каталог площадок.**

Текущие адреса на этом ПК (localhost):

| Сервис | URL |
|---|---|
| Каталог (публичный) | http://127.0.0.1:8090 |
...
Пошагово несколько Moodle, заведение и «синхронизация» учёток:
[multi-moodle-accounts.md](multi-moodle-accounts.md).
...
### В агрегаторе (каталог и обнаружение)

1. Студенты: http://127.0.0.1:8090 — вкладки факультетов и карточки кафедр.
2. Админка: http://127.0.0.1:8090/admin — факультеты (без URL), кафедры с адресом
   (можно «без факультета»), drag-and-drop порядка и вложенности.
...
4. «Проверить» в админке: зелёный = признаки Moodle в HTML; жёлтый = сервер
   отвечает; красный = сеть/HTTP/TLS.
5. «Сохранить» пишет `catalog.json` в Docker volume (и синхронизирует probe-список).
...
Администратор сети
   └─► Агрегатор                         ← контейнер aggregator
         каталог факультетов/кафедр, проверка URL
```
